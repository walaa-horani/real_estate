-- =============================================================================
-- Seed data for Supabase SQL Editor. Run AFTER deploy_schema.sql.
-- Safe to re-run: plans upsert by slug, demo org/properties skipped if
-- 'skyline-properties' already exists.
-- =============================================================================
...
-- No demo auth.users/owner is created here — create a real user via
-- Authentication -> Users in the dashboard, then run:
--   select public.create_organization_with_owner('your-org-slug', 'Your Org Name', 'pro');
-- to attach it as owner of a fresh org, or manually insert into
-- organization_members with that user's id and this demo org's id.
...
declare
  ...
  loft_id uuid;
begin
  if exists (select 1 from public.organizations where slug = 'skyline-properties') then
    return;
  end if;
